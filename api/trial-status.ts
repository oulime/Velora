 ...
 *
 * Returns **200** only when the Supabase admin client is configured and `get_trial_usage` RPC succeeds.
 *
 ...
 *
 * @see ../api/trialShared.ts — `handleTrialStatus`, `TRIAL_ERROR_MISSING_SERVICE_ROLE_KEY`
 */
import type { VercelRequest, VercelResponse } from "@vercel/node";
import { handleTrialStatus } from "./trialShared.js";

export default async function handler(
  req: VercelRequest,
  res: VercelResponse
): Promise<void> {
  await handleTrialStatus(req, res);
}
